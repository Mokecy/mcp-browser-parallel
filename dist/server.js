...
export function createServer(config = {}) {
    const server = new Server({ name: 'mcp-browser-parallel', version: '1.1.0' }, { capabilities: { tools: {} } });
    // ───── Tool Definitions ─────
        ...
        try {
            switch (name) {
                ...
                case 'page_wait': {
                    ...
                    if (args?.time) {
                        const waitMs = Math.min(30000, args.time * 1000);
                    ...
                    if (args?.textGone) {
                        await instance.page.getByText(args.textGone).first().waitFor({ state: 'hidden', timeout });
                        return text(`✅ Text disappeared: "${args.textGone}"`);
                    }
                    if (args?.text) {
                        await instance.page.getByText(args.text).first().waitFor({ state: 'visible', timeout });
                ...
                case 'page_evaluate': {
                    ...
                    await instance.waitForCompletion(async () => {
                        ...
                        result = await instance.page.evaluate(func);
                    });
                ...
                case 'page_file_upload': {
                    ...
                    if (fileChooserState)
                        instance.clearModalState(fileChooserState);
                ...
                case 'page_mouse_drag_xy': {
                    ...
                    await instance.waitForCompletion(async () => {
                        ...
                        await page.mouse.move(args?.endX, args?.endY);
                        await page.mouse.up();
                    });
                ...
                case 'page_generate_locator': {
                    ...
                    try {
                        ...
                        const code = await locator.toCode?.();
                        if (code) {
