...
export function createServer(config = {}) {
    const server = new Server({ name: 'mcp-browser-parallel', version: '1.0.0' }, { capabilities: { tools: {} } });
    // ───── Tool Definitions ─────
        ...
        try {
            switch (name) {
                ...
                case 'browser_connect': {
                    ...
                    return text(`✅ Connected to Chrome at ${cdpUrl}\n` +
                ...
                case 'instance_create': {
                    ...
                    const url = args?.url;
                    const cloneAuth = args?.cloneAuth ?? true;
                    ...
                    return text(`✅ Instance "${id}" created\n` +
                        `🌐 URL: ${info.url}\n` +
                        `📄 Title: ${info.title}\n` +
                ...
                case 'instance_close': {
                    await manager.closeInstance(args?.instanceId);
                ...
                // === Page Operations ===
                case 'page_navigate': {
                    const instance = manager.getInstance(args?.instanceId);
                    const url = args?.url;
                ...
                case 'page_snapshot': {
                    ...
                    return text(`Page snapshot for "${instance.id}" (${instance.page.url()}):\n\n${snapshotText}\n\n` +
                        `Use [ref=eN] values to interact with elements via page_click, page_fill, etc.`);
                }
                case 'page_click': {
                    const instance = manager.getInstance(args?.instanceId);
                    const locator = getLocator(instance, args?.ref);
                    const options = {};
                    if (args?.button)
                        options.button = args.button;
                    if (args?.doubleClick)
                ...
                case 'page_type': {
                    ...
                    await instance.page.keyboard.type(args?.text, {
                        delay: args?.delay || 50,
                    });
                ...
                case 'page_press_key': {
                    const instance = manager.getInstance(args?.instanceId);
                    await instance.page.keyboard.press(args?.key);
                ...
                case 'page_wait': {
                    ...
                    if (args?.time) {
                        await instance.page.waitForTimeout(args.time * 1000);
                        return text(`✅ Waited ${args.time} seconds`);
                    }
                    if (args?.text) {
                        await instance.page.getByText(args.text).waitFor({ timeout });
                        return text(`✅ Text appeared: "${args.text}"`);
                    }
                    if (args?.textGone) {
                        await instance.page.getByText(args.textGone).waitFor({ state: 'hidden', timeout });
                ...
                case 'page_evaluate': {
                    const instance = manager.getInstance(args?.instanceId);
                    const result = await instance.page.evaluate(args?.expression);
                    return text(`Result:\n${JSON.stringify(result, null, 2)}`);
                }
                case 'page_maximize': {
                    await manager.maximizeWindow(args?.instanceId);
                ...
                case 'page_drag': {
                    const instance = manager.getInstance(args?.instanceId);
                    const source = getLocator(instance, args?.startRef);
                    const target = getLocator(instance, args?.endRef);
                ...
                case 'page_file_upload': {
                    ...
                    return text(`✅ Uploaded ${(args?.paths).length} file(s) to [ref=${args?.ref}]`);
                }
                case 'page_fill_form': {
                    const instance = manager.getInstance(args?.instanceId);
                ...
                case 'page_network_requests': {
                    ...
                    const lines = requests.map(r => `${r.method} ${r.status ?? '...'} ${r.url.substring(0, 120)}`);
                ...
                case 'page_mouse_click_xy': {
                    const instance = manager.getInstance(args?.instanceId);
                    await instance.page.mouse.click(args?.x, args?.y, { button: args?.button || 'left' });
                    return text(`✅ Clicked at (${args?.x}, ${args?.y})`);
                }
                case 'page_mouse_move_xy': {
                    const instance = manager.getInstance(args?.instanceId);
                    await instance.page.mouse.move(args?.x, args?.y);
                ...
                case 'page_mouse_drag_xy': {
                    ...
                    await page.mouse.move(args?.endX, args?.endY, { steps: 10 });
                    await page.mouse.up();
                    return text(`✅ Dragged from (${args?.startX},${args?.startY}) to (${args?.endX},${args?.endY})`);
                }
                case 'page_pdf_save': {
                    const instance = manager.getInstance(args?.instanceId);
                    await instance.page.pdf({ path: args?.filePath });
                ...
                case 'page_verify_element_visible': {
                    ...
                    const name = args?.accessibleName;
                    try {
                        const locator = name
                            ? instance.page.getByRole(role, { name })
                            : instance.page.getByRole(role);
                ...
                case 'page_generate_locator': {
                    const instance = manager.getInstance(args?.instanceId);
                    const info = instance.refMap.get(args?.ref);
                    if (!info)
                        return error(`Ref "${args?.ref}" not found.`);
                    let locatorStr;
                    if (info.name) {
                ...
                case 'page_stop_tracing': {
                    const instance = manager.getInstance(args?.instanceId);
                    await instance.context.tracing.stop({ path: args?.filePath });
